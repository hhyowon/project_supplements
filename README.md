...
- 
## 📃콘텐츠 : 다이어트 보조제 추천 사이트 
- 홈페이지 이용객의 설문조사를 기반하여 다이어트보조제를 추천해주는 웹 사이트 입니다.
...
```
 프로젝트 진행 전부터 여름인  만큼 다이어트 보조제에 대한 관심이 있었습니다. 그래서 다이어트 보조제 사이트를 의뢰한 팀의 프로젝트를 담당하고 싶어 했고 수주 제안서를 가지고 치열한 PT경쟁 끝에 이 프로젝트를 담당하게 되었습니다. 내가 원하는 웹사이트를  구현하는 것이 아니라 수주-의뢰 관계에서 진행하는 프로젝트는 처음이라 더 큰 책임감을 느꼈습니다. 우선 의뢰팀이 이 웹사이트에서 보여주고자 하는 것이 정확히 무엇이고 어떤 기능을 중요시하는지  파악했습니다. 이렇게 하라는 건가? 싶은 사항들은 따로 엑셀로 정리하여 우리팀의 PM에게  문의하고 팀의 각 PM끼리  회의 후 더 정확한 방향성을 얻을 수 있었습니다. 이러한 과정을 통해 커뮤니케이션의 중요성을 느끼게 되었습니다.

 이번 프로젝트를 진행하면서 정말 간단하게 보이지만 항상 사용되고 있는 기능들을  정확히 잘 구현하는 것이 프로젝트의 완성도를 높여 주는 중요한 요소라는 것을 다시 한번 깨닫게 되었습니다. 이 프로젝트에서 대부분 기본적으로 들어가 있는 search기능과  pagination기능 부분을 담당했었습니다.
