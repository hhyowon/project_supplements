...
<body>
    <% String surveyTypeId="" ; 
        ...
        Set<String> displayedOptions = new HashSet<>();
                ...
                <div class="container mx-auto"
                    ...
                    <tbody id="surveyboth">
                        ...
                        for (int i=0; i < resultList.size(); i++) { 
                            ...
                            if(!selectsurveyoptresultList.isEmpty()) { 
                                HashMap selOptMap=(HashMap) selectsurveyoptresultList.get(0); 
                                selsurOpt=(String) selOptMap.get("SURVEY_OPT"); 
                            ...
                            <div>
                                ...
                                <label name="SURVEY_QUESTION_ID" value="<%= surveyQuestionId %>">
                                    <%= surveyQuestion %>
                                </label>
                            </div>
                            <% 
                            ...
                            %>
                                ...
                                <select name="<%= surveyQuestionId %>">
                                    ...
                                    <% 
                                        for (int j=0; j < selectsurveyoptresultList.size(); j++) { 
                                            HashMap optRecord = (HashMap) selectsurveyoptresultList.get(j);
                                            String optSurveyOpt = (String) optRecord.get("SURVEY_OPT");
                                            String optSurveyOptId = (String) optRecord.get("SURVEY_OPT_ID");

                                            // optSurveyOpt가 이미 출력된 옵션인지 확인
                                            if (!displayedOptions.contains(optSurveyOpt)) { 
                                    %>
                                        <option value="<%= optSurveyOptId %>"><%= optSurveyOpt %></option>
                                    <%
                                            displayedOptions.add(optSurveyOpt); // Set에 추가하여 중복을 방지
                                        }
                                    }
                                    %>
                                </select>
